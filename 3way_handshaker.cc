...
static uint32_t attemptedConnections;
...
int Connection::bind(const addrinfo *addr) {
 ...
 * Create the socket
 */
int Connection::socket(const char *host, const char *port) {
...
{
public:
    ConnectionPool(const int size, const string host, const string port): _host(host), _port(port), _size(size), _epollFd(0), _connection(), _events(NULL), _usePoll(false), _pollFd(NULL), _connected(0) {
  ...
  void connect() {
    int count = 0;
    for (vector<Connection>::iterator it = _connection.begin();
         it != _connection.end(); ++it) {
      ++count;
  ...
  void state() {
    ...
    for (vector<Connection>::iterator it = _connection.begin();
      ...
      if (info.tcpi_state == TCP_ESTABLISHED) {
        ...
        //cout << "slot: " << count << ", state: " << (int) info.tcpi_state << " counter: " << it->_counter << " time: " << time(NULL) - it->_time << endl;
        if (now - it->_time > 5) {
          it->close();
          // connect again
          if (it->_state == Connection::NOT_CONNECTED) {
  ...
  void recycle() {
    ...
    for (int i = 0; i < num; ++i) {
      if (_events[i].events & EPOLLIN || _events[i].events & EPOLLOUT ) {
        ...
        if (info.tcpi_state == TCP_ESTABLISHED) {
          if (verbose) cout << "established" << endl;
...
{
  cerr << "USAGE: " << prog << "[-c concurent_connections] [-t total_connections] [-p port] [-v] hostname" << endl;
...
int main(int argc, char **argv)
{
  ...
  cout << endl
       ...
       << "Established connections per second:      " << (double)establishedConnections / double((double)duration * (double)1000) << endl;
  return 0;
}
